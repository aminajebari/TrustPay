...
const ChatPanel: React.FC<ChatPanelProps> = ({
    ...
    const markdownComponents: Components = {
        ...
        h1: ({ children }) => <h1 className="text-lg font-bold mb-2 mt-3 first:mt-0 text-[#3B82F6]">{children}</h1>,
        h2: ({ children }) => <h2 className="text-base font-bold mb-2 mt-3 first:mt-0 text-[#60A5FA]">{children}</h2>,
        h3: ({ children }) => <h3 className="text-sm font-bold mb-1 mt-2 first:mt-0 text-[#93C5FD]">{children}</h3>,
        ...
        code: ({ inline, children, className }: { inline?: boolean; children?: React.ReactNode; className?: string }) =>
            inline ? (
                <code className="bg-slate-700/50 px-1.5 py-0.5 rounded text-[#60A5FA] font-mono text-xs break-all">
                    {children}
                </code>
            ) : (
        ...
        a: ({ href, children }) => (
            <a href={href} target="_blank" rel="noopener noreferrer" className="text-[#60A5FA] hover:text-[#3B82F6] underline break-all transition-colors duration-300">
                {children}
            </a>
        ),
        blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-[#3B82F6] pl-3 italic my-2 text-slate-300 bg-[#1A56DB]/5 py-2 rounded-r">
                {children}
            </blockquote>
        ),
    ...
    return (
        <div className={`${isPayrollOpen ? 'hidden lg:flex lg:w-2/3' : 'w-full'} min-h-[50vh] max-h-[80vh] transition-all duration-300 flex flex-col bg-linear-to-br from-slate-900/50 to-slate-800/30 border border-[#1A56DB]/20 rounded-2xl backdrop-blur-md overflow-hidden shadow-2xl hover:shadow-[#1A56DB]/20 relative group`}>
            {/* Animated border glow */}
            <div className="absolute inset-0 rounded-2xl bg-linear-to-r from-[#1A56DB]/20 via-[#3B82F6]/20 to-[#60A5FA]/20 opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-500 -z-10" />
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <div className="w-10 h-10 bg-linear-to-br from-[#1A56DB] to-[#3B82F6] rounded-full flex items-center justify-center shadow-lg shadow-[#1A56DB]/50 animate-pulse">
                                <Bot className="w-6 h-6 text-white" />
                            </div>
                            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-[#3B82F6] rounded-full border-2 border-slate-900 animate-ping" />
                        </div>
                        <div>
                    ...
                    <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full animate-pulse ${publicKey ? 'bg-[#3B82F6] shadow-lg shadow-[#3B82F6]/50' : 'bg-yellow-500 shadow-lg shadow-yellow-500/50'}`} />
                ...
                {messages.map((msg, index) => (
                    ...
                    >
                        <div className={`max-w-[85%] sm:max-w-[70%] group/message ${msg.role === 'user'
                                ? 'bg-linear-to-r from-[#1A56DB] to-[#3B82F6]'
                            ...
                            <div className={`absolute ${msg.role === 'user' ? '-right-2 -top-2' : '-left-2 -top-2'} w-8 h-8 rounded-full ${msg.role === 'user'
                                    ? 'bg-linear-to-br from-[#3B82F6] to-[#1A56DB]'
                                    : 'bg-linear-to-br from-[#1A56DB] to-[#3B82F6]'
                            ...
                            <div className={`text-xs sm:text-sm leading-relaxed ${msg.role === 'user' ? 'text-white' : 'text-white'} break-all`}>
                                {msg.role === 'user' ? (
                            ...
                            <p className={`text-[10px] sm:text-xs mt-1 sm:mt-2 opacity-60 ${msg.role === 'user' ? 'text-white' : 'text-slate-300'}`}>
            ...
            <div className="p-3 sm:p-6 border-t border-slate-800/50 shrink-0 bg-linear-to-r from-slate-900/80 to-slate-800/60 backdrop-blur-sm">
                        ...
                        <div className="flex gap-2 sm:gap-3">
                            <div className="flex-1 relative group">
                                <input
                                    ...
                                    className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm sm:text-base text-white placeholder-slate-500 focus:outline-none focus:border-[#3B82F6] focus:bg-slate-800/70 transition-all duration-300 disabled:opacity-50 backdrop-blur-sm"
                                />
                                <div className="absolute inset-0 rounded-xl bg-linear-to-r from-[#1A56DB]/20 to-[#3B82F6]/20 opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-300 -z-10" />
                            </div>
                            <button
                                type="submit"
                                disabled={!userApiKey.trim()}
                                className="relative group px-4 sm:px-6 py-2 sm:py-3 bg-linear-to-r from-[#1A56DB] to-[#3B82F6] hover:from-[#3B82F6] hover:to-[#1A56DB] text-white rounded-xl font-medium transition-all duration-300 transform hover:scale-105 flex items-center gap-2 disabled:opacity-50 disabled:hover:scale-100 shadow-lg hover:shadow-xl overflow-hidden whitespace-nowrap"
                            >
                ...
                <div className="flex items-center justify-between mt-2">
                    <p className="text-[10px] sm:text-xs text-slate-500">
                        Wallet: {publicKey ? (
                            <span className="text-[#60A5FA] font-mono">{publicKey.toBase58().slice(0, 6)}...</span>
                        ) : (
                            <span className="text-yellow-500">Not connected</span>
                        )}
                    </p>
                    <div className="flex items-center gap-1">
                        <div className="w-1 h-1 bg-[#1A56DB] rounded-full animate-pulse" />
                        <div className="w-1 h-1 bg-[#3B82F6] rounded-full animate-pulse" style={{ animationDelay: '0.2s' }} />
                        <div className="w-1 h-1 bg-[#60A5FA] rounded-full animate-pulse" style={{ animationDelay: '0.4s' }} />
                    </div>
                </div>
            </div>
